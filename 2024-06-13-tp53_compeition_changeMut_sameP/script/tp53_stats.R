...
alldat <- alldat %>% filter(mutRate != 0.375)
...
total_reps = completed_reps %>% filter(corrBlock == 0 & year == 46 & mutRate == 1) %>%
...
tp53clones_noCorrection <- tp53clones %>% filter(corrBlock == 0) %>%
  group_by(year, mutRate) %>%
  mutate(replicate = row_number()-1) %>%
  ungroup

cat("tp53clones_noCorrection\n")
print(tp53clones_noCorrection)

# full list of replicate/corrBlock/year/mutRate combinations
...
cat("tp53clones_full\n")
print(tp53clones_full, n = Inf)

### randomly sample lost simulations from the no correction case

# How many missing reps for each parameter combination
...
#predefined_replicates <- read_csv()
...
for (i in 1:nrow(missing_reps)){
  ...
  filter_mutRate <- missing_reps$mutRate[i]
  
  sampled_data <- tp53clones_full %>%
    filter(corrBlock == 0) %>%
    filter(year == filter_year) %>%
    filter(mutRate == filter_mutRate) %>%
...
write.csv(tp53clones_sampled, glue("/net/gs/vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/dat/tp53clones_sampled_mutRate.csv"), row.names = FALSE)

cat("tp53clones_sampled\n")
print(tp53clones_sampled, n=200)
...
write.csv(tp53clones_sampled_46, glue("/net/gs/vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/dat/tp53clones_sampled_46_mutRate.csv"), row.names = FALSE)
...
write.csv(tp53clones_sampled_summarized, glue("/net/gs/vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/dat/tp53clones_sampled_summarized_mutRate.csv"), row.names = FALSE)
...
#   geom_line(size = 1) +
#   #facet_wrap(~corrBlock, scales = "fixed", nrow = 1, labeller = label_bquote(corrBlock == 0 ~ "No correction", italic(p) ~ "=" ~ .(corrBlock))) +
...
#   ) +
#   # geom_richtext(data = dat_label, aes(label = custom_labels[as.character(mutRate)]), 
#   #               color = "black", label.size = NA, fill = NA,
#   #               label.margin = unit(1, "pt"),
#   #               label.padding = unit(1, "pt"),
#   #               hjust = -0.1, show.legend = FALSE, size = 8/.pt) +
# show(plot)
# 
# if(cluster){
#   ggsave(glue("/net/gs/vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/results/tp53_tissueProportion_stat.png"), plot, width = 6, height = 1.35, units = "in", dpi = 300)
# }else{
#   ggsave(glue("/Volumes/gs-vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/results/tp53_tissueProportion_stat.png"), plot, width = 4.5, height = 2, units = "in", dpi = 300)
# }
# 
# q()
...
#   mutRate = unique(tp53clones$mutRate),
#   year = all_years
# )
# 
# mean_proportions <- combinations %>%
#   left_join(mean_proportions, by = c("corrBlock", "year", "mutRate")) %>%
#   mutate(
...
# custom_breaks <- seq(0,46,by=10)
# 
# plot <- ggplot(mean_proportions, aes(x = year, y = mean_proportion, color = as.factor(mutRate), group = mutRate)) +
#   geom_line() +
...
# 
# if(cluster){
#   ggsave(glue("/net/gs/vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/results/tp53_tissueProportion.png"), plot, width = 6, height = 2, units = "in", dpi = 300)
# }else{
#   ggsave(glue("/Volumes/gs-vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/results/tp53_tissueProportion.png"), plot, width = 6, height = 2, units = "in", dpi = 300)
# }
# 
# q()
# 
