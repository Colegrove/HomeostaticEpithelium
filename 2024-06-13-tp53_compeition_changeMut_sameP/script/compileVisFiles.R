...
nreplicates = 180 # 100 total replicates for each, but due to cluster issues mid-run, re-runs were performed at values > 100.
#nreplicates = 3
replicate = 0:nreplicates
mutRates = c((0.375),(0.5),(1), (2), (2.666))
#mutRates = c((1), (2), (2.666))
...
alldat <- foreach(block = block_values, .combine = "rbind") %:%
            foreach(mutRate = mutRates, .combine = "rbind") %:%
              foreach(rep = replicate, .combine = "rbind") %:%
                foreach(corrTime = corrTimes, .combine = "rbind") %:%
                  foreach(timestep = timesteps, .combine = "rbind") %do%  {

  f <- glue("VisFile_block_{block}_growth_1.0_sigma_{sigmas[1]}_dose_{doses[1]}_p53_{p53block}_mutrate_{mutRate}_correctionTime_{corrTime}_{rep}.{timestep}.txt")
  cat("Opening file:", f, "\n")
