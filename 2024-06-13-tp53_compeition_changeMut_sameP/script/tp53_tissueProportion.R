...
cat('missing reps')
print(missing_reps, n = Inf)

results <- list()
sampled_reps_list <- list()
...
for (i in 1:nrow(missing_reps)){
  ...
  sampled_count <- tp53clones_full %>%
    ...
    #pull(count) %>%
    sample_n(missing_replicates, replace=TRUE)
  
  sampled_reps <- sampled_count$replicate
  sampled_count <- sampled_count$count
  
  # Append results to the list
  results[[i]] <- data.frame(
    corrBlock = rep(filter_corrBlock, missing_replicates),
    year = rep(filter_year, missing_replicates),
    mutRate = rep(filter_mutRate, missing_replicates),
    count = sampled_count
  )
  
  sampled_reps_list[[i]] <- data.frame(
    corrBlock = rep(filter_corrBlock, missing_replicates),
    year = rep(filter_year, missing_replicates),
    mutRate = rep(filter_mutRate, missing_replicates),
    replicate = sampled_reps
  )
  
  
}

reps_dataframe <- bind_rows(sampled_reps_list)

cat('sampled reps\n')
print(reps_dataframe)
...
full_reps_summary <- combinations_full %>%
  left_join(tp53clones_sampled, by = c("corrBlock", "year", "mutRate", "replicate"))
...
if(cluster){
  ...
  ggsave(glue("/net/gs/vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/results/tp53_tissueProportion_noFacetLabels.png"), plot_noLabels, width = 6, height = 1.15, units = "in", dpi = 300)
  ...
  ggsave(glue("/Volumes/gs-vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/results/tp53_tissueProportion_noFacetLabels.png"), plot_noLabels, width = 6, height = 1, units = "in", dpi = 300)
