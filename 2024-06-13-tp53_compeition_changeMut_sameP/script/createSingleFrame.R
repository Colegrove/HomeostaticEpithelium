...
# replicates_noCorr <- c(163, 170,  89,  61, 140, 141)
# replicates_0.01 <- c(48, 81, 59, 50, 92, 96)
# replicates_0.1 <- c(51, 72, 66, 62,  3, 50)
# replicates_0.01_noCorrSampled <- c(83, 56, 26, 60)
# replicates_0.1_noCorrSampled <- c(176)
# corrBlocks_list <- c(0,0.01,0.1, 0.011, 0.11)

replicates_noCorr <- c(82, 111,  92,  88, 139,  25)
replicates_0.01 <- c(92, 16, 43,  5, 23, 29)
replicates_0.1 <- c(12, 60,  3, 19, 81, 20)
replicates_0.01_noCorrSampled <- c(61, 135, 172, 167, 104, 162)
#replicates_0.1_noCorrSampled <- c(160)
...
plotCorrectionEvents <- function(df, output_folder){
  ...
  for (k in corrBlocks_list){
    ...
    # if(k==0.11){ ## if 0.1 samples do not have timepoints, resample from no correction
    ...
    for (j in reps_list){
      ...
      df_subset <- subset(df, year==46 & replicate==j & corrBlock==k & mutRate == 0.5)
