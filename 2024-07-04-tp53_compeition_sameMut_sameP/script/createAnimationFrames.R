...
plotCorrectionEvents <- function(df, output_folder){
  ...
  for (k in unique(df$corrBlock)){
    for (j in unique(df$replicate)){
      cat(glue("Saving corrected blocking probability: {k}, replicate: {j}"),"\n")
      count = 0
      for (i in unique(df$year)){
        count = count + 1
        df_subset <- subset(df, year==i & replicate==j & corrBlock==k)
        ...
        frame_folder <- paste(paste(output_folder, "block_", k, "_rep_", j, "/", sep = ""))
        #cat(paste(frame_folder, "\n"))
        frame_out <- paste(frame_folder, paste("frame_", count, "_block_", k, "_rep_", j, ".png", sep=""), sep="")
        #cat(paste(frame_out, "\n"))
