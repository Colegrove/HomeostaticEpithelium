...
plotCorrectionEvents <- function(df, output_folder){
  print(unique(df$corrBlock))
  if (!dir.exists(output_folder)){
    dir.create(output_folder)
  }
  for (k in unique(df$corrBlock)){
    for (f in unique(df$FAp53block)){
      for (j in sort(unique(df$replicate))){
        cat(glue("Saving corrected blocking probability: {k}, FAtp53block: {f}, replicate: {j}"),"\n")
        count = 0
        for (i in unique(df$year)){
          count = count + 1
          df_subset <- subset(df, year==i & replicate==j & corrBlock==k & FAp53block==f)
          ...
          frame_folder <- paste(paste(output_folder, "block_", k, "_FAtp53block_", f, "_rep_", j, "/", sep = ""))
          #cat(paste(frame_folder, "\n"))
          frame_out <- paste(frame_folder, paste("frame_", count, "_block_", k, "_FAtp53block_", f, "_rep_", j, ".png", sep=""), sep="")
          #cat(paste(frame_out, "\n"))
          if (!dir.exists(frame_folder)){
            dir.create(frame_folder)
          }
          ggsave(frame_out, plot=plot, width=6, height=6, dpi=300)
        }
      }
    }
  }
...
if(cluster){
  output_folder <- glue("/net/gs/vol1/home/huntc10/proj/HomeostaticEpithelium/2024-07-24-competition_sameMut_changeP/results/animation_frames/")
}else{
