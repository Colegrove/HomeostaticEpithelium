...
tp53vaf_subsect = tp53subsect %>%
  select(vaf, age) %>%
  mutate(blockProb = "IM_data")

## order simulation data from weakest to strongest block 
tp53vafSim <- tp53vafSim %>%
  arrange(desc(blockProb)) %>%
  mutate(blockProb = as.factor(blockProb)) %>%
  rename(vaf = VAF)
## combine simulation data and IM data
tp53_allVAF <- bind_rows(tp53vafSim, tp53vaf_subsect)
...
## save plot to file
if(cluster){
  ggsave(glue("/net/gs/vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/dat/martincorena_comparison_thresholds_VAF.png"), plot, width = 10, height = 5, units = "in", dpi = 300)
}else{
  ggsave(glue("/Volumes/gs-vol1/home/huntc10/proj/HomeostaticEpithelium/{project_path}/dat/martincorena_comparison_thresholds_VAF.png"), plot, width = 10, height = 5, units = "in", dpi = 300)
}
